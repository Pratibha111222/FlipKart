...
def payment(req):
        if req.user.is_authenticated:
            user = req.user
            allcart=cart.objects.filter(userid=user.id)
            totalamount=0
            for x in allcart:
                orderid=random.randrange(1000, 90000)
                orderdata = Order.objects.create(orderid=orderid,productid=x.productid,userid=x.userid,qty=x.qty)
                orderdata.save()
            ...
            msg=f"Hi {user}, Thank you for using pour services. \nTotal amount paid=Rs.{totalamount}/-"
...
def showorders(req):
    ...
    context = {"udsername": req.user, "orderdata": orderdata, "totalamount": totalamount} 
